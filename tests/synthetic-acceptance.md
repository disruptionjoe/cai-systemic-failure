...
- Direct mount resolves identity, governance, status, roadmap, Lanes, evidence,
...
- A record that knowingly relies on an earlier supporting source revision while
  ...
  (`SF-SRL-0001`).
- A record that names a correction route but whose admitted source or explicit
...
- CAI-directed automation is active only under the explicit live CapacityOS
  System grant and remains narrowed by repository governance, Lanes, and
  closed-action gates.
