# Test independent observation and stop-condition refusals
...
<synthetic_fixture id="DTCX-ORC-CSC-DUAL-REFUSAL-01">
- Reuse decision `T-reuse` occurs at `T0` under changed condition `C-new`.
- `A-old` remains active and is reused at `T0`; no retirement or replacement
  event is admitted.
...
Draft `SF-DTCX-ORC-CSC-DUAL-REFUSAL-0001.md` with exactly:
...
2. `# SF Decision-Time ORC CSC Dual Refusal 0001`.
...
5. `## Control Results` copying exactly these five records, once each:
   ...
   `control: SF-ORC-0001 | result: FAIL | reason: K-one routing and dispute occur only at T1 after T-reuse is final | no_other_effect: reuse, owner authority, validation timing, and stop condition remain separate`
...
7. `## Verdict` with exactly `REFUSE QUALIFICATION — SF-ORC-0001 AND
   SF-CSC-0001 FAIL INDEPENDENTLY; TC, DRA, AND VBT PASS ONLY AS SUPPLIED
   SYNTHETIC PREMISES`.
8. `## Candidate Effect` with exactly `PROPOSE ORC-CSC DUAL-REFUSAL
