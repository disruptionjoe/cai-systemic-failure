...
objects distinct. The patrol-verification-to-correction test closed as bounded-
...
effect incomplete. Bounded Alaska Airlines Flight 261 record `SF-0032` and
ValuJet Flight 592 record `SF-0033` retain distinct component, maintenance,
custody, operation, oversight, standing, and consequence layers. A partial
Alaska 261 trace now preserves the source-local relation from lubrication task
and interval through end-play detection opportunity, thread wear, jackscrew and
trim-system failure, pitch-control loss, crew response, standing, and
consequence while keeping task, measurement, decision, approval, surveillance,
correction, and control-effect custody incomplete. A bounded Alaska 261 /
...
is routed as an answerable mechanism-design inquiry. `SF-0002` adds a second
...
`SF-0003` adds a non-aerospace Flint water crisis record from public official
...
`SF-ABS-0004` then tests a non-synthetic warning/communication false-positive
class using the Hawaii false ballistic-missile alert official source set. The
candidate refuses the case as positive evidence because the sources center
false alerting, drill procedure, alert origination, safeguards, and correction-
message delay rather than source-backed assumption reuse under changed
conditions.
`SF-ABS-0005` then tests a non-synthetic single-owner execution false-positive
class using the Amtrak Train 188 official source set. The candidate refuses the
case as positive evidence because the sources center situational awareness,
task management, speed compliance, train-control protection, occupant
protection, and emergency response rather than source-backed assumption reuse
under changed conditions.
`SF-ABS-0006` then tests a non-synthetic slow-degradation and infrastructure-
capacity false-positive class using the I-35W Mississippi River bridge-collapse
official source set. The candidate refuses the case as positive evidence
because the sources center inadequate gusset-plate capacity, original design
...
provisional status remain unchanged.
`SF-CEC-0001` then tests composite-evidence coherence with a synthetic
fixture. The candidate refuses positive qualification when individually
...
provisional.
`SF-DTCX-DUAL-REFUSAL-0001` then applies the same five controls to a second
synthetic fixture. DRA and VBT refuse qualification independently while TC,
ORC, and CSC pass only as supplied premises. `SF-VBT-BRIDGED-COVERAGE-0001`
...
`SF-DTCX-TC-VBT-DUAL-REFUSAL-0001` then applies the controls to a sixth
synthetic fixture. TC and VBT refuse qualification independently while DRA,
ORC, and CSC pass only as supplied premises.
`SF-VBT-SPLIT-COMPONENT-COVERAGE-0001` separately shows that coverage of one
historical parent component does not prove coverage after a pre-decision
revision replaces it with two current named components. Complete current
